...
def run_macs_for_k(file_path, k1_k2_combinations):
    results = []
    for k1, k2 in k1_k2_combinations:
        graph = EvrptwGraph(file_path)
        macs = AntColonySystem(graph, ants_num=10, max_iter=3, alpha=1, beta=2, q0=0.9, k1=k1, k2=k2)
...
def find_best_k1_k2(results_df):
    ...
    best_k1_k2 = sorted_df.head(1)
    return best_k1_k2[['k1', 'k2']]
...
def tuning(directory_path):
    ...
    k2_values = np.arange(1,5)

    num_cores = 6
    k1_k2_combinations = [(k1, k2) for k1 in k1_values for k2 in k2_values]
    chunks = [k1_k2_combinations[i::num_cores] for i in range(num_cores)]
    ...
    best_k1_k2 = find_best_k1_k2(results_df)
    best_k1_k2.to_csv('best_value_k1_k2.csv', index=False)
