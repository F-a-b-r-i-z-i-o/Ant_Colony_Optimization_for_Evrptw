...
def load_configs(general_config_file, aco_config_file):
    configs = {}
...
def run_macs(run, file_path, max_iter, ants_num, alpha, beta, q0, rho, k1, k2, apply_local_search):
    graph = EvrptwGraph(file_path, rho)
    macs_instance = AntColonySystem(graph, ants_num, max_iter, alpha, beta, q0, k1, k2, apply_local_search)
    _, c_test_history, time_history, penalty_history, improvement_iter_history, num_vehicles, improvement_path = macs_instance._ACS_DIST_G(apply_local_search)
...
def save_path_details_to_csv(path_details, filename):
    df = pd.DataFrame(path_details)
    df['coordinates'] = df['coordinates'].apply(lambda coords: ";".join([f"({x},{y})" for x, y in coords]))
    dask_df = dd.from_pandas(df, npartitions=1)
    dask_df.to_csv(filename, index=False, single_file=True)


def calculate_and_append_average(filename):
    ...
    for filename in sorted(os.listdir(abs_directory_path)):
        if filename.endswith(".txt"):
            ...
            for i in range(num_runs):
                delayed_run = delayed(run_macs)(i, file_path, max_iter, ants_num, alpha, beta, q0, rho, k1, k2, apply_local_search)
            ...
            if save_path_details:
                ...
                path_details_filename = os.path.join(instance_folder, "path_results.csv")
